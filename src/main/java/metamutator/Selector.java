...
//		}
	...
	public static <E> void generateSelector(CtElement element, E initialChoice, int selectorId, EnumSet<?> possibleChoices, String prefix ) {
		...
		//CtTypeReference<Object> fieldType = element.getFactory().Type().createTypeParameterReference(Selector.class.getName());
		CtTypeReference<Object> classRefs = element.getFactory().Type().createTypeParameterReference(choiceClass.getCanonicalName());
		
		
		CtTypeReference<Object> fieldType = element.getFactory().Type().createTypeParameterReference(Selector.class.getCanonicalName());
		fieldType.addActualTypeArgument(classRefs);
		//fieldType.addActualTypeArgument(genericFieldType);
